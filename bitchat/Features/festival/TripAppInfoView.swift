...
struct TripAppInfoView: View {
    ...
    private var infoContent: some View {
        VStack(alignment: .leading, spacing: 24) {
            ...
            // Features
            VStack(alignment: .leading, spacing: 16) {
                TripSectionHeader(Strings.Features.title)

                TripFeatureRow(info: Strings.Features.offlineComm)

                TripFeatureRow(info: Strings.Features.encryption)

                TripFeatureRow(info: Strings.Features.extendedRange)

                TripFeatureRow(info: Strings.Features.favorites)

                TripFeatureRow(info: Strings.Features.geohash)

                TripFeatureRow(info: Strings.Features.mentions)
            }

            // Privacy
            VStack(alignment: .leading, spacing: 16) {
                TripSectionHeader(Strings.Privacy.title)

                TripFeatureRow(info: Strings.Privacy.noTracking)

                TripFeatureRow(info: Strings.Privacy.ephemeral)
            ...
            // Upstream bitchat settings (tor, bridge/gateway, relays, theme,
            // notification previews, panic wipe) — added in the 2026-09 merge.
            advancedSettingsRow
    ...
    /// Entry row for upstream's settings sheet.
        ...
        Button(action: { showAdvancedSettings = true }) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text("Advanced mesh settings")
                        .font(.system(size: 14, weight: .semibold, design: .monospaced))
                        .foregroundColor(textColor)
                    Text("Tor, internet bridge & gateway, relays, theme, notification previews, emergency wipe")
    ...
    /// Upstream's AppInfoView needs LocationChannelsModel in the environment
    /// (sheets can drop inherited environment objects, #1558), so inject the
    /// runtime's models explicitly.
    @ViewBuilder
    private var advancedSettingsSheet: some View {
        if let runtime = TripChatState.shared.appRuntime {
            AppInfoView(
                topologyProvider: { runtime.appChromeModel.meshTopologyDisplayModel() },
                onPanicWipe: { runtime.appChromeModel.panicClearAllData() }
            )
...
struct NetworkPrivacySection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            TripSectionHeader("NETWORK SETTINGS")
...
struct DataDisclosureSection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            TripSectionHeader("DATA & THIRD PARTIES")
                ...
                DisclosureGroup(isExpanded: $isExpanded) {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                TripBulletPoint("Your public key (not your real identity)")
                                TripBulletPoint("Approximate location (~150m) when using location channels")
                                TripBulletPoint("Encrypted message content (unreadable)")
                                TripBulletPoint("Timestamps")
                            }
                        }
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                TripBulletPoint("Your real name, email, or phone number")
                                TripBulletPoint("Decrypted message content")
                                TripBulletPoint("Your exact GPS location")
                                TripBulletPoint("Your IP address (when Tor is enabled)")
                            }
                        }
...
struct TripFeatureRow: View {
    let info: TripAppInfoFeatureInfo
    @Environment(\.colorScheme) var colorScheme
